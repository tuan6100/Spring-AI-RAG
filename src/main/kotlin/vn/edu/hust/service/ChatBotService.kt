package vn.edu.hust.service

import io.github.oshai.kotlinlogging.KotlinLogging
import org.springframework.ai.chat.client.ChatClient
import org.springframework.ai.chat.client.advisor.MessageChatMemoryAdvisor
import org.springframework.ai.chat.client.advisor.SimpleLoggerAdvisor
import org.springframework.ai.chat.client.advisor.vectorstore.QuestionAnswerAdvisor
import org.springframework.ai.chat.memory.ChatMemory
import org.springframework.ai.chat.prompt.PromptTemplate
import org.springframework.ai.template.st.StTemplateRenderer
import org.springframework.ai.vectorstore.SearchRequest
import org.springframework.ai.vectorstore.VectorStore
import org.springframework.beans.factory.annotation.Autowired
import org.springframework.stereotype.Service
import reactor.core.publisher.Flux
import vn.edu.hust.controller.require.IChatBotService
import vn.edu.hust.controller.require.IRagService


@Service
class ChatBotService(
    @field:Autowired private final val chatClient: ChatClient,
    @field:Autowired private final val ragService: IRagService,
    @field:Autowired private val vectorStore: VectorStore,
    @field:Autowired private val chatMemory: ChatMemory
) : IChatBotService {

    private val logger = KotlinLogging.logger {}

    override suspend fun generateAnswer(query: String): Flux<String> {
        if (query.isBlank()) {
            return Flux.error(IllegalArgumentException("Query cannot be null or empty."))
        }
        logger.info { "Query: $query" }
        try {
            val documents = ragService.search(query)
            var answer = ""
            if (documents.isNullOrEmpty()) {
                answer = "No relevant information found for your query."
                logger.info { answer }
                return Flux.just(answer)
            }
            val context: String = documents.joinToString("\n\n") { it.text.toString() }.trim()
            if (context.isBlank()) {
                answer = "No context available to generate an answer."
                logger.info { answer }
                return Flux.just(answer)
            }
            logger.info { "Context: $context \n" }
            val promptTemplate: PromptTemplate = PromptTemplate.builder()
                .renderer(StTemplateRenderer.builder().startDelimiterToken('<').endDelimiterToken('>').build())
                .template(
                    """
            $query
            Context information is below.
			---------------------
			$context
			---------------------
			Given the context information and no prior knowledge, answer the query.
			Follow these rules:
			1. If the answer is not in the context, just say that you don't know.
			2. Avoid statements like "Based on the context..." or "The provided information...".
            """.trimIndent()
                )
                .build()
            return chatClient.prompt(promptTemplate.template)
                .advisors {
                    MessageChatMemoryAdvisor.builder(chatMemory).build()
                    QuestionAnswerAdvisor.builder(vectorStore).build()
                    SearchRequest.builder().similarityThreshold(0.5).topK(9).build()
                    SimpleLoggerAdvisor()
                }
                .user(query)
                .stream()
                .content()
                .map { it.toString() }
                .doOnNext { answerPart -> answer += answerPart }
                .doOnComplete { logger.info { "Answer: $answer \n" } }
        } catch (e: Exception) {
            e.printStackTrace()
            logger.error { "Error occurred: ${e.message}" }
            return Flux.error(RuntimeException("Sorry, the service is currently unavailable. Please try again later."))
        }
    }

}